...
import type { FlowData } from '@/lib/mockFlowData'
...
export function generateFlowLayout(flowData: FlowData): {
  nodes: Node[]
  edges: Edge[]
} {
  const nodes: Node[] = []
  ...
  nodes.push({
    ...
    data: { prompt: flowData.basePrompt },
  })
  ...
  nodes.push({
    ...
    data: { prompt: flowData.enhancedPrompt },
  })
  ...
  // 3. Target Group flows with iterations
  const numTargetGroups = flowData.targetGroups.length
  const targetGroupStartY = VERTICAL_SPACING * 2

  flowData.targetGroups.forEach((targetGroup, tgIndex) => {
